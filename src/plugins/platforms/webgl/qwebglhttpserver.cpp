...
QWebGLHttpServer::QWebGLHttpServer(QWebGLWebSocketServer *webSocketServer,
                                   QObject *parent)
    : QObject(parent), d_ptr(new QWebGLHttpServerPrivate) {
  Q_D(QWebGLHttpServer);
  d->webSocketServer = webSocketServer;

  connect(&d->server, &QTcpServer::newConnection, this,
          &QWebGLHttpServer::clientConnected);
}
...
void QWebGLHttpServer::setCustomRequestDevice(const QString &name,
                                              QIODevice *device) {
  Q_D(QWebGLHttpServer);
...
void QWebGLHttpServer::clientConnected() {
  ...
  connect(socket, &QTcpSocket::disconnected, this,
          &QWebGLHttpServer::clientDisconnected);
...
void QWebGLHttpServer::answerClient(QTcpSocket *socket, const QUrl &url) {
  ...
  qCDebug(lc, "%s requested: %s", qPrintable(socket->localAddress().toString()),
          qPrintable(path));
  ...
  QByteArray answer =
      QByteArrayLiteral("HTTP/1.1 404 Not Found\r\n"
                        ...
                        "</body>"
                        "</html>");
  const auto addData = [&answer](const QByteArray &contentType,
                                 const QByteArray &data) {
    answer = QByteArrayLiteral("HTTP/1.0 200 OK \r\n");
    QByteArray ret;
    const auto dataSize = QString::number(data.size()).toUtf8();
    answer += QByteArrayLiteral("Content-Type: ") + contentType +
              QByteArrayLiteral("\r\n") +
  ...
  if (path == QLatin1String("/")) {
    ...
    // QT_WEBGL_WEBSOCKETSERVER_EXTERNAL

    const QUrl externalUrl(
    ...
    if (!externalUrl.isValid() || externalUrl.isEmpty() ||
        !(externalUrl.scheme() == "ws" || externalUrl.scheme() == "wss")) {
            host = url.host();
...
bool HttpRequest::readMethod(QTcpSocket *socket) {
  ...
  if (finished) {
    if (fragment == "HEAD")
      ...
      qCWarning(
          lc, "QWebGLHttpServer::HttpRequest::readMethod: Invalid operation %s",
          fragment.data());
...
bool HttpRequest::readUrl(QTcpSocket *socket) {
  ...
  if (finished) {
    if (!fragment.startsWith("/")) {
      qCWarning(lc,
                "QWebGLHttpServer::HttpRequest::readUrl: Invalid URL path %s",
                fragment.constData());
      return false;
    }
...
bool HttpRequest::readStatus(QTcpSocket *socket) {
  ...
  if (finished) {
    if (!std::isdigit(fragment.at(fragment.size() - 3)) ||
      ...
      qCWarning(lc,
                "QWebGLHttpServer::HttpRequest::::readStatus: Invalid version");
      return false;
    }
